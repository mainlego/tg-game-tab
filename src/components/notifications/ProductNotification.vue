...
<template>
  <div v-if="showNotification" class="product-notification" @click="handleClick">
    <div class="notification-content" :style="{ background: activeProduct?.gradient || 'linear-gradient(140.83deg, rgb(140, 96, 227) 0%, rgb(73, 51, 131) 100%)' }">
      <img :src="getProductImage(activeProduct)" alt="Новый продукт" class="notification-image">
...
watch(() => store.passiveIncome, (newValue, oldValue) => {
...
const checkAvailableProducts = () => {
  // Если нет продуктов, выходим
  if (!allProducts.value.length) return

  // Ищем продукт, который стал доступен, но еще не активирован
  const newlyAvailableProduct = allProducts.value.find(product => {
    // Проверяем, что продукт стал доступен по доходу
    const isAvailable = store.passiveIncome >= product.requiredIncome

    // Проверяем, что продукт еще не активирован
    const isAlreadyClaimed = userClaims.value.some(claim => claim.productId === product._id)

    return isAvailable && !isAlreadyClaimed
  })

  if (newlyAvailableProduct) {
    // Запоминаем доступный продукт и показываем уведомление
